...
Cast(VALEUR as varchar) c11, 
dbo.GetMontant([NO,EMPL],[CODE,PAIE],[PAIE,NO],isNull(Tire,'')), 
Case
	When dbo.GetMontant([NO,EMPL],[CODE,PAIE],[PAIE,NO],isNull(Tire,'')) = '' Then ''
	Else isNull(Cast(Cast(((dbo.GetMontant([NO,EMPL],[CODE,PAIE],[PAIE,NO],isNull(Tire,'')) / VALEUR)/taux) as numeric(8,2)) as varchar),'')
End c13,
...
FROM CUMULALL 
JOIN MS_SOFE_TAG ON CUMULALL.[CODE,PAIE] = MS_SOFE_TAG.MS
LEFT JOIN SOFE_Titre_Emploi ON SOFE_Titre_Emploi.matricule = CUMULALL.[NO,EMPL]
...
WHERE CUMULALL.[CODE,PAIE] in ('351', '363')
...
Case
	When dbo.GetMontant([NO,EMPL],[CODE,PAIE],[PAIE,NO],isNull(Tire,'')) = '' Then ''
	Else isNull(Cast(Cast((dbo.GetMontant([NO,EMPL],[CODE,PAIE],[PAIE,NO],isNull(Tire,'')) / VALEUR) as numeric(8,2)) as varchar),'')
End c13, 
